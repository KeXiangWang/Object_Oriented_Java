...
public class InputHandler implements Runnable {
	...
	public InputHandler(ArrayList<Taxi> taxiList, RequestQueue requestQueue, TaxiGUI taxiGUI, TaxiMap taxiMap, Scanner in) {
		/**	@REQUIRES: 
		  	taxiList!=null;
		 	requestQueue!=null;
		 	taxiGUI!=null;
	...
	public void run() {
		...
		@EFFECTS: 
			normal_behavior:
				this.requestQueue.size()>=\old(this.requestQueue.size());
				the status of some roads changed.;
	...
	public Boolean parseRequest(String str) {
		...
		@EFFECTS: 
			normal_behavior:
				this.requestQueue.size()>=\old(this.requestQueue.size());
				str is a valid request ==> \result=true;
				str is not a valid request ==> \result=false;
			exception_behavior(Throwable e):
				\result=false;
	...
	public Boolean parseSetRoad(String str) {
		...
		@EFFECTS: 
			normal_behavior:
				this.requestQueue.size()>=\old(this.requestQueue.size());
				str is a valid setRoad request ==> the road'status changed ==> \result=true;
	...
	public Boolean parseCheckTaxi(String str) {
		...
		@EFFECTS: 
			normal_behavior:
				print the taxi information;
				str is a valid check request ==> the road'status changed ==> \result=true;
				str is not a valid setRoad request==> \result=false;
			exception_behavior(Throwable e):
				\result=false;
		*/
