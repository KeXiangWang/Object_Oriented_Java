...
//	private int[][] fastestDistance; // the fastest distance
	...
	public TaxiMap() {
		 ...
		 *		(\all int i; 0<=i<TaxiSys.SQUARESIZE; pointMap[i] != null);
		 *		(\all int i; 0<=i<TaxiSys.SQUARESIZE; taxiHere[i] != null);
		 */
		taxiHere = new ArrayList[TaxiSys.SQUARESIZE];
		pointMap = new Point[TaxiSys.SQUARESIZE];
...
//	}
	...
	public ArrayList<Taxi> getAmbientTaxi(Request request) {
		 ...
		 * @MODIFIES: None;
		 * @EFFECTS: 
		 * 		\result == ambientTaxiList;
		 * 		AmbientTaxiList consists of all the dispatchable taxi for the request in the ambient 4*4-square.
		 */
