...
class guigv {
	...
	public static void AddFlowN(int x1, int y1, int x2, int y2, int n) {// 增加一个道路流量
		synchronized (guigv.flowmap) {
			// 查询之前的流量数量
			int count = 0;
			count = guigv.flowmap.get(Key(x1, y1, x2, y2)) == null ? 0 : guigv.flowmap.get(Key(x1, y1, x2, y2));
			// 添加流量
			guigv.flowmap.put(Key(x1, y1, x2, y2), count + n);
			guigv.flowmap.put(Key(x2, y2, x1, y1), count + n);
		}
	}
	...
	public static int GetFlowN(int x1, int y1, int x2, int y2) {// 查询流量信息
		synchronized (guigv.flowmap) {
			return guigv.flowmap.get(Key(x1, y1, x2, y2)) == null ? 0 : guigv.flowmap.get(Key(x1, y1, x2, y2));
		}
	}
...
class TaxiGUI {// GUI接口类
	public void LoadMap(int[][] map, int size) {
		...
		Thread th = new Thread(new Runnable() {
			public void run() {
				long timewindow = System.currentTimeMillis() + 100;// 时间窗设置为500ms
				gv.stay(100);
				timewindow = timewindow + 500;
				// int timecount = 0;// 计时
				while (true) {
					if (timewindow - System.currentTimeMillis() > 100) {
						gv.stay(100);
						guigv.drawboard.repaint();
					} else {
						int temp = (timewindow - System.currentTimeMillis()) > 0
								? (int) (timewindow - System.currentTimeMillis())
								: 0;
						gv.stay(temp);
						timewindow = timewindow + 500;
						guigv.drawboard.repaint();
						guigv.ClearFlow();
//						System.out.println("fghjk" + System.currentTimeMillis());
					}
				}
			}
		});
...
//		if(index==0) {
//			System.out.println("cao"+gt.x+"  "+gt.y+""+point);
//		}
		gt.x = point.x;
		gt.y = point.y;
		gt.status = status;
			...
			if (di == 0) {// 在同一水平线上
				...
				if (status == 0) {// 关闭
					...
					synchronized (guigv.m.graph) {
						guigv.m.graph[p1.x * 80 + p1.y][p2.x * 80 + p2.y] = 0;
						guigv.m.graph[p2.x * 80 + p2.y][p1.x * 80 + p1.y] = 0;
					}
				} else if (status == 1) {// 打开
					...
					synchronized (guigv.m.graph) {
						guigv.m.graph[p1.x * 80 + p1.y][p2.x * 80 + p2.y] = 1;
						guigv.m.graph[p2.x * 80 + p2.y][p1.x * 80 + p1.y] = 1;
					}
				}
